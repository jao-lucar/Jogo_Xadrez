...
def criar_tabuleiro():
    ...
    for i in range(len(tabuleiro)):  # para cada linha em tabuleiro
        for _ in range(8):  # se repete 8 vezes pois esse é o numero de colunas do tabuleiro, é aqui que se define isso
            ...
            tabuleiro[i].append([pygame.Rect(x, y, 85, 85), 0]) # cada indice da matriz do tabuleiro recebe uma lista
...
def desenha_tabuleiro(janela: pygame.Surface, tabuleiro):
    #
    cores = [
    ...
    for i in tabuleiro.values(): # para cada linha em tabuleiro
        for j in i: # j = cada lista com j[0] = quadrado e j[1] com 0 ou objeto pygame com a imagem da peça
            ...
            # j[1] == imagens ou o valor 0, isso depende daonde a peça esta posicionada, 0 == nao existe peça aqui
            # j[0][0] == pixels x do quadrado e j[0][1] == pixels y  do quadrado
            # so colocarei as peças aonde j[1] != 0, ou seja, aonde existe uma peça

            if j[1] != 0:
                janela.blit(j[1], (j[0][0], j[0][1]))
